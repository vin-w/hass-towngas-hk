...
| 實體 | 類型 | 單位 | 描述 |
|------|------|------|------|
| `sensor.gas_consumption` | 感測器 | MJ | 當月煤氣消耗量（月中為估計值） |
...
### 屬性（`sensor.gas_consumption`）

| 屬性 | 描述 |
|------|------|
| `account_no` | 中華煤氣帳戶號碼 |
| `readings` | 過去 8 個月消耗歷史 |
| `bills` | 過去 4 張帳單（日期及 HKD 金額） |
...
[Gas icons created by Freepik - Flaticon](https://www.flaticon.com/free-icons/gas)
