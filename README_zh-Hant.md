...
中華煤氣要求每次登入都需要 **OTP（一次性密碼）驗證**。這是中華煤氣的安全措施。

### 資料更新方式

資料每 **30 天** 擷取一次（對應每月帳單週期）。在更新間隔期間，快取資料會立即顯示 — 無需網路連線。

重啟時，整合會立即載入快取資料。僅在 30 天間隔到期時才擷取最新資料。

### 儲存密碼選項

在設定和重新驗證時，您可以勾選**「儲存密碼以自動重新整理」**：

| 選項 | 行為 |
|------|------|
| **已儲存** | 密碼在重新驗證時自動填入 — 您只需確認並輸入驗證碼 |
| **未儲存** | 每次重新驗證時必須手動輸入密碼 |

兩種選項都遵循相同的 30 天更新週期。差異僅在於重新驗證時的便利性。

### 何時需要重新驗證

- **每 30 天** — 更新間隔到期時，協調器需要新的登入憑證
- **連線逾時** — HA 在「設定 → 整合」中顯示「重新驗證」通知
- **強制重新整理** — 點擊按鈕手動取得最新資料（如連線逾時需重新驗證）

### 重新驗證流程

1. 在修復通知中點擊**重新驗證**（或點擊**強制重新整理**）
2. 輸入密碼（已儲存則自動填入，否則手動輸入）
3. ☐ 如需方便下次使用，勾選**儲存密碼以自動重新整理**
4. 輸入寄送到您電郵的 **6位數驗證碼**
5. 完成 — 已載入最新資料
...
本軟體按「現狀」提供，不附帶任何明示或暗示的保證。本項目是一個獨立的非官方整合，與香港中華煤氣有限公司無關。使用本整合風險自負。作者不對因使用本整合而產生的任何問題承擔責任。
